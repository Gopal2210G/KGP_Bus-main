...
const initializeTables = async () => {
    try {
        ...
        await pool.query(`
            ...
            CREATE INDEX IF NOT EXISTS idx_loc_bus_composite 
            ON locations(bus_id, timestamp DESC);
            
            CREATE INDEX IF NOT EXISTS idx_user_loc_composite 
            ON user_locations(user_id, timestamp DESC);
            
            CREATE INDEX IF NOT EXISTS idx_routes_bus_stop 
            ON routes(bus_id, stop_order);

            CREATE INDEX IF NOT EXISTS idx_bus_drivers_user 
            ON bus_drivers(user_id);
        `);

        console.log('Database tables and composite indexes initialized');
    } catch (err) {
