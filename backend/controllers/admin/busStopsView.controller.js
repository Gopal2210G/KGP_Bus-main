    ...
    try {
        ...
        const result = await pool.query(`
            WITH BusRoutes AS (
                ...
                FROM kgp_bus_track.routes r1
                JOIN kgp_bus_track.routes r2 ON r1.bus_id = r2.bus_id
            ...
            RouteSegments AS (
                ...
                FROM FromStops fs
                JOIN kgp_bus_track.routes r ON fs.bus_id = r.bus_id AND r.bus_stop_id = $2
                ...
                AND NOT EXISTS (
                    SELECT 1 FROM kgp_bus_track.routes r2
            ...
            SELECT 
                ...
                b.currentRep,
                b.totalRep,
                COALESCE(bst.start_time, '08:00:00') AS start_time,
                COALESCE(bst.rep_no, b.currentRep, 1) AS rep_no,
                ...
                rs.from_time,
                rs.to_time,
                (ct.id IS NOT NULL) AS is_cancelled,
                ct.reason AS cancelled_reason
            FROM RouteSegments rs
            JOIN kgp_bus_track.buses b ON rs.bus_id = b.id
            LEFT JOIN kgp_bus_track.bus_start_time bst ON b.id = bst.bus_id AND (bst.rep_no = b.currentRep OR b.currentRep IS NULL)
            LEFT JOIN kgp_bus_track.cancelled_trips ct ON b.id = ct.bus_id AND (ct.rep_no = bst.rep_no OR ct.rep_no = COALESCE(b.currentRep, 1))
            JOIN kgp_bus_track.routes r1 ON rs.bus_id = r1.bus_id AND rs.from_order = r1.stop_order
            JOIN kgp_bus_track.routes r2 ON rs.bus_id = r2.bus_id AND rs.to_order = r2.stop_order
            JOIN kgp_bus_track.bus_stops fs ON r1.bus_stop_id = fs.id
            JOIN kgp_bus_track.bus_stops ts ON r2.bus_stop_id = ts.id
        ...
        const busesWithTimes = buses.map((bus, index) => {
            ...
            const startTimeStr = bus.start_time || '08:00:00';
            ...
            return {
                ...
                name: bus.bus_name,
                currentTrip: bus.currentrep || 1,
                totalTrips: bus.totalrep || 1,
                tripNumber: bus.rep_no || 1,
                is_cancelled: Boolean(bus.is_cancelled),
                cancelled_reason: bus.cancelled_reason,
                route: {
                    fromStop: {
                        id: bus.from_stop_id,
