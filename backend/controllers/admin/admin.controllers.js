...
export const getStatistics = asyncHandler(async (req, res) => {
    // Gather statistics in a single database query to minimize network roundtrips
    const result = await pool.query(`
        SELECT
            (SELECT COUNT(*) FROM users WHERE role = 'user') AS total_users,
            (SELECT COUNT(DISTINCT user_id) FROM user_locations WHERE timestamp > NOW() - INTERVAL '24 hours') AS active_users,
            (SELECT COUNT(*) FROM buses) AS total_buses,
            (SELECT COUNT(DISTINCT bus_id) FROM locations WHERE timestamp > NOW() - INTERVAL '24 hours') AS active_buses,
            (SELECT COUNT(*) FROM bus_stops) AS total_stops,
            (SELECT COUNT(DISTINCT bus_id) FROM routes) AS total_routes,
            (SELECT COUNT(*) FROM users WHERE role = 'driver') AS total_drivers,
            (SELECT COUNT(*) FROM locations WHERE timestamp > NOW() - INTERVAL '1 hour') AS recent_locations
    `);
    ...
    const stats = {
        ...
        totalBuses: parseInt(row.total_buses) || 0,
        activeBuses: parseInt(row.active_buses) || 0,
        totalStops: parseInt(row.total_stops) || 0,
        totalRoutes: parseInt(row.total_routes) || 0,
        totalDrivers: parseInt(row.total_drivers) || 0,
        recentLocations: parseInt(row.recent_locations) || 0
    };
...
// Get 7-day schedule for a specific bus ID
export const getBusSchedule = asyncHandler(async (req, res) => {
    const { id } = req.params;

    try {
        const busResult = await pool.query('SELECT * FROM buses WHERE id = $1', [id]);
        ...
        console.error(`Error fetching schedule for bus ${id}:`, error);
