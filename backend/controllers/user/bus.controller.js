...
export const getBusLocation = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length === 0) {
            ...
            return res
                .status(200)
                .json(new ApiResponse(200, null, "Bus location not found"));
        }
...
export const getBusRoute = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const cancelledCheck = await pool.query(
            'SELECT * FROM kgp_bus_track.cancelled_trips WHERE bus_id = $1 AND rep_no = $2',
            [busId, currentRep]
        );
        const isCancelled = cancelledCheck.rows.length > 0;
        const cancelledReason = isCancelled ? cancelledCheck.rows[0].reason : null;
