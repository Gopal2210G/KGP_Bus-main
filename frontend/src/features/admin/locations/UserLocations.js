...
function UserLocations({ user }) {
  ...
  const fetchUserLocations = useCallback(async (isSilent = false) => {
    try {
  ...
  const [userPannedMap, setUserPannedMap] = useState(false);
  
  // Real-time WebSockets integration + polling fallback
  useEffect(() => {
    fetchUserLocations(false);

    // Subscribe to real-time WebSocket updates
    const { subscribeToAdminTracking } = require('../../../utils/socket.js');
    const unsubscribe = subscribeToAdminTracking(({ type, data }) => {
      if (!data) return;
      setRawLocations(prev => {
        const userId = data.userId || data.user_id || data.busId;
        const exists = prev.some(item => (item.user_id || item.id) === userId);
        if (exists) {
          return prev.map(item => ((item.user_id || item.id) === userId ? { ...item, ...data } : item));
        } else {
          return [data, ...prev];
        }
      });
    ...
    const intervalId = setInterval(() => {
      fetchUserLocations(true); // Silent background fallback polling
    }, 15000);
  ...
  return (
    <div className="user-locations">
      ...
      <div className="action-buttons">
        <button 
          onClick={() => fetchUserLocations(false)} 
          disabled={loading} 
          ...
          <MapContainer
            ...
            whenReady={(mapInstance) => {
              if (mapInstance && mapInstance.target) {
                ...
                mapInstance.target.on('dragstart', () => setUserPannedMap(true));
              }
            }}
          >
            <TileLayer
                    ...
                    <td>
                      <span className={`status-indicator ${status}`}>
                        {status === 'active' ? 'Active now' : 
                         ...
                         status === 'today' ? 'Active today' : 
                         status === 'unknown' ? 'No GPS data' : 'Not recent'}
                      </span>
                    </td>
                    <td>
